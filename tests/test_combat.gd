...
func run() -> void:
	...
	var hp0 := z.hp
	main._resolve_melee(me, Look.PUNCH_L, main.PUNCH)
	...
	for i in 60:
		z.down_t = 0.0
		z.position = home + Vector2(14, 0)
		main._resolve_melee(me, Look.KICK, [26.0, 1.0, 0.5, 0.1, 0.0])
	...
	for i in 30:
		...
		main._resolve_melee(me, Look.SWING, [26.0, 1.0, 0.5, 0.1, 0.0])
	...
	var styles := {}
	for i in 400:
		styles[main.death_style("axe")] = true
	check(styles.has("behead") and styles.has("arm"), "axes behead and take arms")
	check(main.death_style("knife") == "stab", "knives stab")
	check(main.death_style("gun") == "burst", "a shot to the head bursts it")
	var n0: int = main.zombies.size()
	w.hp = 1
	w.position = home + Vector2(14, 0)
	main._resolve_melee(me, Look.SWING, [26.0, 5.0, 0.5, 0.1, 0.0])
