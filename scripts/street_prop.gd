...
## A parked motorbike at its real size (about 1.9 m long), in one of the
## models above. Some carry a delivery box, a front basket or a motorbike
## taxi rider's vest; some lie where they fell.
func _motorbike() -> void:
	...
	match model:
		"wave":
			_bike_wave(col)
		"click":
			_bike_click(col, true)
		"ev":
			_bike_click(col, false)
		"scoopy":
			_bike_scoopy(col)
		"pcx":
			_bike_pcx(col)
	...
	if not fallen:
		if extra < 2 and model in ["wave", "click", "pcx"]:  # a delivery rider's box
			...
			draw_rect(Rect2(-15, -25, 9, 8.5), bc)
			draw_rect(Rect2(-15, -25, 9, 1.5), bc.lightened(0.25))
			draw_rect(Rect2(-13.5, -22, 6, 2.5), Color(1, 1, 1, 0.8))
		elif extra == 2 and model in ["wave", "scoopy"]:  # a wire basket on the front
			draw_rect(Rect2(9, -23, 6, 4), Color("3a3c40"), false, 0.7)
			draw_line(Vector2(11, -23), Vector2(11, -19), Color("3a3c40"), 0.5)
			draw_line(Vector2(13, -23), Vector2(13, -19), Color("3a3c40"), 0.5)
		elif extra == 3 and model in ["wave", "click"]:  # a win rider's orange vest left on the seat
			draw_colored_polygon(PackedVector2Array([Vector2(-11, -16.5), Vector2(-5, -17), Vector2(-4, -14), Vector2(-10, -13.5)]),
					Color("e8782a"))
	draw_set_transform(Vector2.ZERO)


func _wheel(x: float, r: float, spokes := false) -> void:
	draw_circle(Vector2(x, -r), r, Color("141414"))
	draw_circle(Vector2(x, -r), r * 0.45, Color("8a8e92"))
	if spokes:
		for i in 4:
			var a := i * PI / 4
			draw_line(Vector2(x, -r) + Vector2.from_angle(a) * r * 0.75, Vector2(x, -r) - Vector2.from_angle(a) * r * 0.75,
					Color("6a6e72"), 0.4)
	draw_circle(Vector2(x, -r), r * 0.18, Color("2a2a2a"))
...
func _tail(x: float, y: float) -> void:
	draw_rect(Rect2(x, y, 1.4, 2), Color("d8302a"))
	draw_rect(Rect2(x - 0.4, y + 2.6, 2.2, 1.8), Color("ecebe4"))  # plate


func _mirror(x: float, y: float) -> void:
	draw_line(Vector2(x, y), Vector2(x - 1, y - 3), Color("3a3a3a"), 0.6)
	draw_circle(Vector2(x - 1, y - 3.2), 0.9, Color("5a6068"))


## Honda Wave: the underbone everyone's family rides. Open floor between the wheels.
func _bike_wave(col: Color) -> void:
	_wheel(-10, 4.5)
	_wheel(11, 4.5)
	draw_rect(Rect2(-14, -6.5, 8, 1.6), Color("6a6e72"))  # exhaust
	draw_rect(Rect2(-7, -9.5, 6, 4.5), Color("3a3c40"))  # engine
	draw_rect(Rect2(-3, -9, 9, 2), Color("26282c"))  # floor
	_poly([Vector2(-15.5, -10), Vector2(-12.5, -14.5), Vector2(-2.5, -13.5), Vector2(-1.5, -8.5), Vector2(-8, -8), Vector2(-14.5, -7.5)], col)
	draw_line(Vector2(-14.8, -10.2), Vector2(-3, -12.6), col.lightened(0.25), 0.7)
	_tail(-16.2, -12)
	_poly([Vector2(-13.5, -14.2), Vector2(-12.5, -16.4), Vector2(-3, -15.8), Vector2(-2, -13.6)], Color("1c1c1e"))
	_poly([Vector2(3.5, -8.5), Vector2(6.5, -8.5), Vector2(9.5, -18.5), Vector2(6, -19.5), Vector2(3, -11)], col)
	draw_line(Vector2(8, -17), Vector2(11, -5), Color("7a7e82"), 1.2)
	_poly([Vector2(7.5, -9.5), Vector2(14.5, -8.5), Vector2(13.5, -10.5), Vector2(9, -11)], col.darkened(0.35))
	draw_line(Vector2(4.5, -21), Vector2(10, -20.2), Color("1a1a1a"), 1.1)
	draw_rect(Rect2(8.6, -19.6, 2.6, 2.2), col.lightened(0.1))
	draw_rect(Rect2(10.6, -19.2, 1.2, 1.6), Color("f4eed0"))
	_mirror(6, -21)
...
func _bike_click(col: Color, petrol: bool) -> void:
	_wheel(-10, 4.3)
	_wheel(11, 4.3)
	if petrol:
		draw_rect(Rect2(-15, -7, 8, 2), Color("5a5e62"))
		draw_rect(Rect2(-15.5, -7.2, 2, 2.4), Color("8a8e92"))
	else:
		draw_circle(Vector2(-10, -4.3), 2.8, Color("4a4e52"))  # hub motor
	_poly([Vector2(-16, -10.5), Vector2(-13, -15), Vector2(-3, -14), Vector2(-1, -9), Vector2(-9, -6.5), Vector2(-15, -7.5)], col)
	_poly([Vector2(-15, -8), Vector2(-9, -6.5), Vector2(-1, -9), Vector2(-1, -8), Vector2(-9, -5.5)], col.darkened(0.3))
	draw_rect(Rect2(-3, -10, 8, 2.5), col.darkened(0.12))  # flat floor
	_tail(-16.6, -12.5)
	_poly([Vector2(-14, -14.6), Vector2(-12.5, -17), Vector2(-3, -16.2), Vector2(-2, -14)], Color("1c1c1e"))
	_poly([Vector2(4, -7.5), Vector2(7.5, -7.5), Vector2(10.5, -19), Vector2(6.5, -20.5), Vector2(3.5, -10)], col)
	_poly([Vector2(8, -9), Vector2(14.5, -8.2), Vector2(13.5, -10), Vector2(9, -10.5)], col.darkened(0.3))
	draw_line(Vector2(8.5, -17), Vector2(11, -5), Color("5a5e62"), 1.1)
	draw_rect(Rect2(5, -22.5, 6.5, 2.4), col)  # bar cover
	draw_line(Vector2(7, -21.4), Vector2(11.5, -21.6), Color("eef4ff"), 0.6)  # LED strip
	draw_line(Vector2(4.5, -22), Vector2(3, -22.3), Color("1a1a1a"), 1.0)
	_mirror(6, -22.5)
	if not petrol:
		draw_line(Vector2(-10, -12), Vector2(-7, -12.8), Color("5ad0ff"), 0.8)  # the blue stripe EVs wear


## Honda Scoopy: round and retro, pastel with a cream panel and a round lamp.
func _bike_scoopy(col: Color) -> void:
	_wheel(-10, 4.3)
	_wheel(11, 4.3)
	draw_rect(Rect2(-14.5, -6.8, 7, 1.8), Color("8a8e92"))
	var body := []
	for i in 13:
		var a := PI + i * PI / 12
		body.append(Vector2(-9, -9) + Vector2(cos(a) * 7, sin(a) * 5.5))
	_poly(body, col)
	draw_rect(Rect2(-16, -9.5, 14, 2.5), col)
	draw_rect(Rect2(-3, -10, 8, 2.5), Color("efe6d4"))  # cream floor panel
	_tail(-16.4, -11.5)
	_poly([Vector2(-14.5, -14), Vector2(-13, -16.5), Vector2(-3.5, -16), Vector2(-2.5, -13.8)], Color("6a4a32"))  # brown seat
	var front := []
	for i in 9:
	...
	_poly(front, col)
	_poly([Vector2(8, -9), Vector2(14.5, -8.2), Vector2(13.5, -10.5), Vector2(9, -11)], col)
	draw_line(Vector2(8.5, -17), Vector2(11, -5), Color("9a9ea2"), 1.1)
	draw_line(Vector2(4.5, -21.5), Vector2(10, -21), Color("d8d8d0"), 1.0)  # chrome bars
	draw_circle(Vector2(9.5, -19), 1.9, Color("d8d8d0"))
	draw_circle(Vector2(9.7, -19), 1.3, Color("fff6d8"))  # round lamp
	_mirror(5.5, -21.5)
...
func _bike_pcx(col: Color) -> void:
	_wheel(-12, 4.8)
	_wheel(12, 4.8)
	draw_rect(Rect2(-17, -7.5, 9, 2.2), Color("4a4e52"))
	draw_rect(Rect2(-17.5, -7.8, 2, 2.8), Color("8a8e92"))
	_poly([Vector2(-18, -11), Vector2(-15, -16), Vector2(-4, -15), Vector2(-1, -9.5), Vector2(-11, -6.5), Vector2(-17, -8)], col)
	_poly([Vector2(-17, -8.5), Vector2(-11, -6.5), Vector2(-1, -9.5), Vector2(-1, -8.5), Vector2(-11, -5.5)], col.darkened(0.4))
	_poly([Vector2(-2, -9.5), Vector2(4, -9.5), Vector2(3, -12.5), Vector2(-1, -12.5)], col.darkened(0.15))  # tunnel
	_tail(-18.6, -13.5)
	_poly([Vector2(-16, -15.8), Vector2(-15, -18.2), Vector2(-9, -17.8), Vector2(-8, -16.2), Vector2(-3, -16), Vector2(-2.5, -14.5)],
			Color("1c1c1e"))  # stepped seat
	_poly([Vector2(3.5, -8), Vector2(8, -8), Vector2(12, -20), Vector2(8, -22), Vector2(3.5, -11)], col)
	_poly([Vector2(8, -22), Vector2(10.5, -26), Vector2(12, -25.5), Vector2(11.5, -21)], Color(0.55, 0.65, 0.75, 0.55))  # screen
	_poly([Vector2(9, -9.5), Vector2(16, -9), Vector2(15, -11), Vector2(10, -11.5)], col.darkened(0.35))
	draw_line(Vector2(10, -18), Vector2(12, -5), Color("5a5e62"), 1.3)
	draw_line(Vector2(5, -22.5), Vector2(9, -22.8), Color("1a1a1a"), 1.1)
	draw_line(Vector2(10.5, -19.5), Vector2(12.8, -18.5), Color("eef4ff"), 0.9)  # LED face
	_mirror(6, -23)


## A sports bike (Honda CBR): sharp fairing, tank, a high pointed tail,
## low bars. Rare, and loud.
func _bike_sport(col: Color) -> void:
	_wheel(-11, 5, true)
	_wheel(12, 5, true)
	draw_line(Vector2(-11, -5), Vector2(-1, -9), Color("3a3c40"), 1.4)  # swingarm
	draw_rect(Rect2(-6, -11, 8, 5), Color("3a3c40"))  # engine
	_poly([Vector2(-7, -12), Vector2(-3, -15), Vector2(2, -14), Vector2(3, -8), Vector2(-5, -6.5)], col.darkened(0.2))
	_poly([Vector2(-17, -18), Vector2(-7, -15), Vector2(-5, -13.5), Vector2(-12, -14.5)], col)  # tail up high
	_poly([Vector2(-10, -16.5), Vector2(-5, -16.8), Vector2(-4, -15.2), Vector2(-9, -15)], Color("1c1c1e"))
	_poly([Vector2(-5, -16.5), Vector2(1, -18.5), Vector2(4, -16.5), Vector2(1, -14), Vector2(-4, -14.5)], col)  # tank
	_poly([Vector2(2, -9), Vector2(3, -17), Vector2(9, -20), Vector2(14, -15), Vector2(11, -10), Vector2(6, -8)], col)  # fairing
	_poly([Vector2(8, -20), Vector2(10, -22), Vector2(12, -18.5)], Color(0.5, 0.6, 0.7, 0.6))
	draw_rect(Rect2(12, -15.8, 2, 1.2), Color("f4eed0"))
	draw_line(Vector2(9, -16), Vector2(12, -5), Color("c8a030"), 1.3)  # gold forks
	_poly([Vector2(-8, -8), Vector2(-15, -12), Vector2(-15.5, -10.5), Vector2(-9, -7)], Color("8a8e92"))  # exhaust under the tail
	draw_rect(Rect2(-17.4, -18, 1.4, 1.6), Color("d8302a"))


## A trail bike (Honda CRF): big thin wheels, a long travel fork, a high
## front fender and a flat seat. Rare.
func _bike_trail(col: Color) -> void:
	_wheel(-11, 5.8, true)
	_wheel(12, 6.2, true)
	draw_line(Vector2(-11, -5.8), Vector2(-2, -11), Color("3a3c40"), 1.3)
	draw_rect(Rect2(-5, -12.5, 6, 5), Color("3a3c40"))
	draw_line(Vector2(-3, -16), Vector2(6, -18), Color("2a2c30"), 1.2)  # frame
	_poly([Vector2(-16, -18.5), Vector2(-3, -17.5), Vector2(-2, -16), Vector2(-14, -16.8)], col)
	_poly([Vector2(-13, -19.5), Vector2(0, -19), Vector2(1, -17.5), Vector2(-12, -18)], Color("1c1c1e"))  # flat seat
	_poly([Vector2(-1, -17), Vector2(4, -20), Vector2(7, -18), Vector2(3, -14)], col)  # tank shrouds
	draw_line(Vector2(7, -21), Vector2(12, -6.2), Color("b8bcc0"), 1.4)  # long fork
	_poly([Vector2(9, -15), Vector2(16, -15.5), Vector2(15, -14), Vector2(10, -13.8)], col)  # high fender
	draw_rect(Rect2(5, -21.5, 3.5, 3.5), Color("e8e8e8"))  # number plate
	draw_line(Vector2(4, -23), Vector2(9, -23), Color("1a1a1a"), 1.1)
	draw_rect(Rect2(8, -20.5, 1.2, 1.4), Color("f4eed0"))
	draw_line(Vector2(-6, -9), Vector2(-15, -14.5), Color("8a8e92"), 1.2)  # high pipe
	draw_rect(Rect2(-16.8, -18.5, 1.4, 1.4), Color("d8302a"))
