...
func _shadow(size: Vector2, offset := Vector2.ZERO) -> void:
	...
	c.draw_set_transform_matrix(_xf * Transform2D(0, Vector2(1, size.y / size.x), 0, offset))
	c.draw_circle(Vector2.ZERO, size.x, Color(0, 0, 0, 0.3))
...
func _car_side(burnt := false) -> void:
	...
	c.draw_colored_polygon(cabin, col.lightened(0.05))
	var glass := Color("121010") if burnt else Color("2a323a")
	c.draw_colored_polygon(PackedVector2Array([Vector2(8.5, -10.5), Vector2(15, -10.5), Vector2(15, -15.2), Vector2(10.7, -15.2)]), glass)
	c.draw_colored_polygon(PackedVector2Array([Vector2(16, -10.5), Vector2(22.5, -10.5), Vector2(20.4, -15.2), Vector2(16, -15.2)]), glass)
	c.draw_rect(Rect2(10, -17, 11, 1.2), col.lightened(0.2))  # roof catching the light
	if taxi:
...
func _car_end() -> void:
	...
	c.draw_rect(Rect2(2.5, -27, 11, 2.5), Color("2a323a"))
	c.draw_rect(Rect2(1, -6, 14, 5), col.darkened(0.3))  # front face
...
func _tuktuk() -> void:
	...
	c.draw_line(Vector2(2, -8), Vector2(2, -18), Color("2a2a2a"), 0.8)
	c.draw_line(Vector2(14.5, -8), Vector2(14.5, -18), Color("2a2a2a"), 0.8)
	c.draw_rect(Rect2(0, -20, 16, 3), col.darkened(0.25))  # canopy
	for i in 8:
...
func _motorbike() -> void:
	BikeArt.draw(c, data.seed, data.get("view", "side"), data.get("dir", 1.0 if data.seed % 2 else -1.0), "all")


func _cart() -> void:
	_shadow(Vector2(8, 2.5))
	c.draw_rect(Rect2(-7, -9, 14, 7), Color("b8bcc0"))
	c.draw_rect(Rect2(-7, -9, 14, 1), Color("dcdfe2"))
	c.draw_rect(Rect2(-5, -14, 10, 5), Color(0.8, 0.9, 0.95, 0.6))  # glass case
	c.draw_rect(Rect2(-4, -12, 3, 2), Color("d89a3a"))
	c.draw_rect(Rect2(0, -12, 3, 2), Color("c85a3a"))
	c.draw_circle(Vector2(-5, -2), 1.6, Color("222222"))
	c.draw_circle(Vector2(5, -2), 1.6, Color("222222"))
	c.draw_line(Vector2(0, -14), Vector2(0, -26), Color("6a6a6a"), 0.8)
	var col: Color = [Color("c83a2e"), Color("2a6ab8"), Color("e8a030")][data.seed % 3]
	for i in 8:
		var a0 := PI + i * PI / 8
		var a1 := PI + (i + 1) * PI / 8
		c.draw_colored_polygon(PackedVector2Array([Vector2(0, -29), Vector2(cos(a0) * 11, -26 + sin(a0) * -2 + 2),
...
func _trash() -> void:
	...
	for i in 3:
		...
		c.draw_circle(p, 2.6, Color("1a1a1c"))
		c.draw_circle(p + Vector2(-0.8, -1), 0.8, Color("4a4a50"))


func _pillar() -> void:
	var concrete := Color("a19d94")
	var top := World.TILE - World.BTS_H + 9.0  # up to the deck underside
	c.draw_rect(Rect2(3, -World.TILE * 2.0 + 2, 10, 4), Color(0, 0, 0, 0.25))
	c.draw_polygon(PackedVector2Array([Vector2(3, 0), Vector2(13, 0), Vector2(12, top), Vector2(4, top)]),
			PackedColorArray([concrete.darkened(0.3), concrete.darkened(0.15), concrete, concrete.lightened(0.05)]))
	c.draw_rect(Rect2(-3, top - 4, 22, 5), concrete.darkened(0.1))
...
func _wreck() -> void:
	...
	if flip:
		...
		c.draw_colored_polygon(PackedVector2Array([Vector2(16, -10.5), Vector2(19, -10.5), Vector2(23, -17), Vector2(20, -17)]),
	...
	if burnt:
		for i in 6:
			...
			c.draw_circle(p, r.randf_range(0.8, 1.8), Color(0.42, 0.2, 0.08, 0.7))  # rust
			c.draw_circle(p + Vector2(1.5, 1), r.randf_range(0.8, 1.6), Color(0.05, 0.04, 0.03, 0.6))  # soot
...
func _glass() -> void:
	...
	for i in 9:
		...
		c.draw_rect(Rect2(p, Vector2(1, 0.6)), Color(0.8, 0.9, 0.95, 0.55))
...
func _papers() -> void:
	...
	for i in r.randi_range(2, 4):
		...
		c.draw_set_transform(p, r.randf_range(-0.8, 0.8), Vector2.ONE)
		c.draw_rect(Rect2(-1.5, -1, 3, 2), Color("d8d4c8") if i % 2 else Color("c8b890"))
	c.draw_set_transform(Vector2.ZERO)
...
func _luggage() -> void:
	...
	c.draw_set_transform(Vector2.ZERO, r.randf_range(-0.6, 0.6), Vector2.ONE)
	c.draw_rect(Rect2(-4, -3, 8, 5), col)
	c.draw_rect(Rect2(-4, -3, 8, 5), col.darkened(0.35), false, 0.5)
	c.draw_rect(Rect2(-1, -3.8, 2, 0.8), Color("2a2a2a"))
	for i in 3:
		c.draw_rect(Rect2(3 + i * 2, -1 + i * 0.6, 2.5, 1.5), Color.from_hsv(r.randf(), 0.4, 0.7))
	c.draw_set_transform(Vector2.ZERO)
...
func _drag() -> void:
	...
	for i in 7:
		...
		c.draw_circle(p, 1.4 - i * 0.12, Color(0.3, 0.03, 0.02, 0.55 - i * 0.05))
...
func _debris() -> void:
	var r := _rng()
	_shadow(Vector2(8, 2))
	c.draw_line(Vector2(-7, -2), Vector2(6, -8), Color("7a5a3a"), 1.6)
	for i in 5:
		var p := Vector2(r.randf_range(-6, 6), r.randf_range(-7, -2))
		c.draw_circle(p, r.randf_range(2, 3.4), Color("1a1a1c") if i % 2 else Color("2a3a5a"))
	c.draw_rect(Rect2(2, -6, 4, 1), Color("c83a2e"))  # a plastic stool, legs up
	c.draw_rect(Rect2(2, -9, 1, 3), Color("a82e24"))
	c.draw_rect(Rect2(5, -9, 1, 3), Color("a82e24"))
...
func _spirit() -> void:
	...
	_shadow(Vector2(4, 1.5))
	c.draw_rect(Rect2(-1, -14, 2, 14), Color("e8e2d4"))  # pillar
	c.draw_rect(Rect2(-5, -16, 10, 2), Color("c8c0b0"))
	c.draw_rect(Rect2(-3.5, -22, 7, 6), Color("e8dcc0"))
	c.draw_rect(Rect2(-1.2, -21, 2.4, 4), Color("8a2a1a"))  # doorway
	c.draw_colored_polygon(PackedVector2Array([Vector2(-5, -22), Vector2(5, -22), Vector2(0, -28)]), Color("b8452a"))
	c.draw_line(Vector2(-5, -22), Vector2(0, -28), gold, 0.6)
	c.draw_line(Vector2(5, -22), Vector2(0, -28), gold, 0.6)
	c.draw_line(Vector2(0, -28), Vector2(0.8, -30), gold, 0.6)
	# Offerings: garlands, a red soda with a straw, little figures.
	c.draw_circle(Vector2(-3.5, -16.5), 0.9, Color("e8c040"))
	c.draw_circle(Vector2(3.5, -16.5), 0.9, Color("e87a3a"))
	c.draw_rect(Rect2(1.5, -18, 1, 2), Color("c8283a"))
	if data.seed % 2 == 0:
		c.draw_line(Vector2(-4, -16), Vector2(4, -16), Color("e8e0a0"), 0.5)
...
func _stall() -> void:
	...
	c.draw_rect(Rect2(-5, -6, 10, 1.5), Color("e8e2d4"))
	c.draw_rect(Rect2(-4, -4.5, 1, 4.5), Color("9a9a9a"))
	c.draw_rect(Rect2(3, -4.5, 1, 4.5), Color("9a9a9a"))
	for i in 3:
		var x := -8.0 + i * 7.0
		if r.randf() < 0.3:
			c.draw_rect(Rect2(x, -1.5, 3, 1.5), tone.darkened(0.2))  # knocked over
		else:
			c.draw_rect(Rect2(x, -3.5, 3, 1.2), tone)
			c.draw_rect(Rect2(x + 0.3, -2.3, 2.4, 2.3), tone.darkened(0.2))
	c.draw_line(Vector2(0, -6), Vector2(0, -22), Color("7a7a7a"), 0.7)
	for i in 8:
		var a0 := PI + i * PI / 8
		var a1 := PI + (i + 1) * PI / 8
		c.draw_colored_polygon(PackedVector2Array([Vector2(0, -25), Vector2(cos(a0) * 12, -21 + sin(a0) * -1.5 + 1.5),
...
func _bin() -> void:
	_shadow(Vector2(4, 1.5))
	c.draw_rect(Rect2(-3, -8, 6, 8), Color("2e6a3a"))
	c.draw_rect(Rect2(-3.4, -9, 6.8, 1.5), Color("245630"))
	c.draw_rect(Rect2(-3, -8, 1.2, 8), Color(1, 1, 1, 0.08))
	if data.seed % 3 == 0:
		c.draw_circle(Vector2(4, -1.5), 2, Color("1a1a1c"))  # a bag that did not fit
...
func _boat() -> void:
	...
	c.draw_set_transform(Vector2.ZERO, 0, Vector2(s, 1))
	var hull := PackedVector2Array([Vector2(-22, -4), Vector2(18, -4), Vector2(24, -8), Vector2(14, -1), Vector2(-20, -1)])
	c.draw_colored_polygon(hull, wood.darkened(0.35) if sunk else wood)
	c.draw_line(Vector2(-21, -4), Vector2(23, -8), wood.lightened(0.2), 0.8)
	if not sunk:
		c.draw_rect(Rect2(-6, -5, 12, 1.5), Color("3a2a1a"))  # thwart
		for i in 3:
			c.draw_circle(Vector2(20 + i * 0.5, -8 - i), 0.8, [Color("e84a6a"), Color("e8c040"), Color("4a9ae8")][i])  # ribbons on the bow
	c.draw_rect(Rect2(-24, -7, 4, 3), Color("5a5a5a"))  # engine on its pole
	c.draw_line(Vector2(-22, -5), Vector2(-32, 1), Color("6a6a6a"), 1.0)
	c.draw_set_transform(Vector2.ZERO)
	c.draw_line(Vector2(-22, 0), Vector2(22, 0), Color(1, 1, 1, 0.15), 0.8)  # waterline
...
func _sandbags() -> void:
	...
	for row in 2:
		for i in 3:
			...
			c.draw_set_transform(p, r.randf_range(-0.1, 0.1), Vector2(1, 0.55))
			c.draw_circle(Vector2.ZERO, 3.4, khaki.darkened(r.randf() * 0.15))
			c.draw_set_transform(Vector2.ZERO)
...
func _barrier() -> void:
	...
	for x in [-6.0, 6.0]:
		c.draw_line(Vector2(x, 0), Vector2(x, -7), Color("5a5a5a"), 0.8)
	for i in 6:
		c.draw_rect(Rect2(-7 + i * 2.33, -7, 2.33, 2.5), Color("c83a2e") if i % 2 == 0 else Color("ece8e0"))
...
func _army() -> void:
	...
	for x in [10.0, 18.0, 26.0]:
		c.draw_line(Vector2(x, -12), Vector2(x, -3), green.darkened(0.25), 0.6)
